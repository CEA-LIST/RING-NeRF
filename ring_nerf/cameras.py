...
class Cameras(TensorDataclass):
    ...
    def _generate_rays_from_coords(
        ...
        #######################################################################################
        ################# HERE IS THE BAISC CHANGE ############################################
        #######################################################################################
        if metadata is not None:
            metadata["directions_norm"] = directions_norm[0].detach()
            metadata["dir"] = directions.detach()
            metadata["dir_x"] = directions_stack[1].detach()
            metadata["dir_y"] = directions_stack[2].detach()#.reshape(sh[1],sh[2],-1)]
        else:
            metadata = {"directions_norm": directions_norm[0].detach()}#,"dir":directions.detach(),"dir_x":directions_stack[1].detach(),"dir_y":directions_stack[2].detach()},#.reshape(sh[1],sh[2],-1)}
            metadata["dir"] = directions.detach()
            metadata["dir_x"] = directions_stack[1].detach()
            metadata["dir_y"] = directions_stack[2].detach()
        #######################################################################################
        #######################################################################################
        #######################################################################################
        return RayBundle(
